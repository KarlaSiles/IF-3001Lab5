...
public class OrdesCircularLinkedController
{
    ...
    public void sortyIDAction(ActionEvent actionEvent) {

            try {
                orderList.sortByProductId(); // Llama al método para ordenar por ID
                tableView.setItems(getData()); // Actualiza la tabla con los datos ordenados
            } catch (ListException ex) {
                alert.setContentText("There was an error in sorting the orders by ID");
                alert.showAndWait();
            }
        }
    ...
    public void getPrevAction(ActionEvent actionEvent) {
        try {
            ...
            try {
                ...
                int previousIndex = (index - 2 + orderList.size()) % orderList.size() + 1; // Si es el primer elemento, vuelve al final
                Node previousNode = orderList.getNode(previousIndex);
                Order previousOrder = (Order) previousNode.getData();

                // Muestra la información del cliente anterior encontrado
                alert.setAlertType(Alert.AlertType.INFORMATION);
                alert.setHeaderText("Previous Order:");
                alert.setContentText(previousOrder.toString());
                alert.showAndWait();
    ...
    public void getNextAction(ActionEvent actionEvent) {
        try {
            ...
            try {
                ...
                int nextIndex = (index % orderList.size()) + 1; // Si es el último elemento, vuelve al principio
                Node nextNode = orderList.getNode(nextIndex);
                Order nextOrder = (Order) nextNode.getData();

                // Muestra la información del siguiente cliente encontrado
                alert.setAlertType(Alert.AlertType.INFORMATION);
                alert.setHeaderText("Next Order:");
                alert.setContentText(nextOrder.toString());
                alert.showAndWait();
    ...
    public void removeFirtsAction(ActionEvent actionEvent) {
        try {
            ...
            if (!orderList.isEmpty()) {
                ...
                alert.setAlertType(Alert.AlertType.INFORMATION);
                alert.setContentText("The first order was removed from the list.");
                alert.showAndWait();
            } else {
                //Muestra un mensaje de alerta si la lista de clientes está vacía
                alert.setAlertType(Alert.AlertType.WARNING);
                alert.setContentText("The order list is empty.");
                alert.showAndWait();
            }
        } catch (ListException e) {
